...
const CameraScreen: React.FC<CameraScreenProps> = ({ onBack, onNext, initialClips = [] }) => {
  ...
  useEffect(() => {
    console.log('[STATE] isRecording:', isRecording, '| clips count:', clips.length);
  ...
  const handleCapturePress = useCallback(async () => {
    console.log('[BUTTON] Record/Stop button pressed');
    console.log('[BUTTON] mode:', mode);
    console.log('[BUTTON] isRecording:', isRecording);
    console.log('[BUTTON] hasPermission:', hasPermission);

    if (mode === CameraModeEnum.Video) {
      if (isRecording) {
        console.log('[BUTTON] ⏹️ Stopping recording...');
        await stopRecording();
      } else {
      ...
      console.log('[BUTTON] 📷 Taking photo...');
  ...
  const handleNextPress = useCallback(() => {
    console.log('➡️ [VERIFICATION] handleNextPress triggered');
    ...
    if (clips.length > 0) {
      console.log('✅ [VERIFICATION] Calling onNext with', clips.length, 'clip(s)');
      console.log('✅ [VERIFICATION] About to navigate to PreviewScreen');
      onNext(clips);
    } else {
      console.log('⚠️ [VERIFICATION FAILED] No clips to send - onNext not called');
    }
  }, [clips, onNext]);
    ...
    <SafeAreaView style={cameraStyles.cameraContainer}>
      <View style={cameraStyles.cameraPreview}>
        ...
        <View style={cameraStyles.speedSelectorOverlay}>
          <SpeedSelector speed={speed} onSpeedChange={(newSpeed: number) => setSpeed(newSpeed as SpeedMultiplier)} disabled={mode === CameraModeEnum.Photo} />
        </View>
      </View>
