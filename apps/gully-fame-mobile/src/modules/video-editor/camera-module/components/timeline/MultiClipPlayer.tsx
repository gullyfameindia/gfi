...
try {
  ...
  ResizeMode = expoVideo.ResizeMode;
} catch (e) {
  console.warn("[MultiClipPlayer] expo-video not available (requires dev build):", (e as any)?.message);
}
...
const MultiClipPlayer: React.FC<MultiClipPlayerProps> = ({
  ...
  const handleVideoLoad = useCallback((clipId: string, status: any) => {
    console.log('[MultiClipPlayer] 🎥 [VERIFICATION] Video loaded - clipId:', clipId, 'isLoaded:', status.isLoaded, 'durationMillis:', status.durationMillis);
    if (status.isLoaded && clipId === currentClipId) {
      
      const localTime = currentClipLocalTime;
      const videoRef = videoRefs.current.get(clipId);
      if (videoRef?.current) {
        console.log('[MultiClipPlayer] 🎥 [VERIFICATION] Seeking to position:', localTime * 1000, 'ms, setting rate:', currentSpeed);
        videoRef.current.setPositionAsync(localTime * 1000).catch(console.warn);
        videoRef.current.setRateAsync(currentSpeed, true).catch(console.warn);
        
        if (isPlaying) {
          console.log('[MultiClipPlayer] 🎥 [VERIFICATION] Starting playback');
          videoRef.current.playAsync().catch(console.warn);
        }
      }
