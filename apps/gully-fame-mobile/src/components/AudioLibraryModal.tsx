...
interface AudioLibraryModalProps {
  ...
  onToggleSave?: (trackId: string) => Promise<void>;
  multiSelect?: boolean;
  selectedTracks?: Set<string>;
  onMultiSelect?: (trackId: string) => void;
}
...
export const AudioLibraryModal: React.FC<AudioLibraryModalProps> = ({
  ...
  const [localSavedTracks, setLocalSavedTracks] = useState<Set<string>>(new Set(savedTracks));
  ...
  const fetchTracks = useCallback(async () => {
    ...
    try {
      ...
      console.log('[AudioLibraryModal] Fetching tracks:', { category: activeCategory, sort: sortOption });

      const result = await musicLibraryService.listAudio(sortOption, page, 50, searchQuery);

      if (result.success && result.data) {
        console.log(`[AudioLibraryModal] Loaded ${result.data.tracks.length} tracks`);
        ...
        if (activeCategory === 'Saved') {
          ...
          setLocalSavedTracks(savedIds);
        }
      } else {
        console.warn('[AudioLibraryModal] Failed to fetch:', result.message);
        setTracks([]);
      }
    } catch (error) {
      console.error('[AudioLibraryModal] Error:', error);
      setTracks([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSelectMusic = useCallback(
    (music: TrackWithStats) => {
      console.log('[AudioLibraryModal] Selected:', music.title);
      if (multiSelect && onMultiSelect) {
        onMultiSelect(music._id);
      } else {
        onSelect(music);
      }
    },
  ...
  const handleToggleSave = useCallback(
    async (trackId: string, e: any) => {
      ...
      try {
        if (onToggleSave) {
          await onToggleSave(trackId);
        }
        setLocalSavedTracks(prev => {
        ...
        console.error('[AudioLibraryModal] Error toggling save:', error);
      }
    },
    [onToggleSave]
  );
  ...
  const renderTrack = ({ item }: { item: TrackWithStats }) => {
    const isSelected = multiSelect ? selectedTracks.has(item._id) : false;
    const isSaved = localSavedTracks.has(item._id);
    ...
    return (
      ...
      >
        ...
        >
          <Svg width="24" height="24" viewBox="0 0 24 24" fill={isSaved ? '#ec9a15' : 'none'}>
            <Path
              d="M19 21L12 16L5 21V5C5 4.46957 5.21071 3.96086 5.58579 3.58579C5.96086 3.21071 6.46957 3 7 3H17C17.5304 3 18.0391 3.21071 18.4142 3.58579C18.7893 3.96086 19 4.46957 19 5V21Z"
              stroke={isSaved ? '#ec9a15' : '#999'}
              strokeWidth="2"
              strokeLinecap="round"
