  ...
  effects.forEach((effect) => {
    switch (effect.type) {
      ...
      case 'vignette':
        ...
        const vignetteAngle = 45 * effect.intensity; 
...
export async function applyOverlaysToVideo(
  ...
  console.log(`🎨 [applyOverlaysToVideo] Processing ${overlays.length} overlays`);
  overlays.forEach((o, i) => {
    console.log(`  [${i}] type: ${o.type}, content: ${String(o.content).substring(0, 50)}, x: ${o.x}, y: ${o.y}, scale: ${o.scale}`);
  });

  
  const imageStickers = overlays.filter((o) => o.type === "image");
  const textOverlays = overlays.filter((o) => o.type === "text" || o.type === "emoji");
  ...
  if (imageStickers.length > 0) {
    ...
    imageStickers.forEach((sticker, index) => {
      ...
      const nextOutput = `v_img_${index}`;
      
      
      
      
      const x = `(W * ${sticker.x || 50} / 100)`;
      const y = `(H * ${sticker.y || 50} / 100)`;
      const scale = sticker.scale || 1;
      
      
      if (scale !== 1) {
        filterChain.push(`[${currentInput}]scale=iw*${scale}:ih*${scale}[${currentInput}_scaled]`);
        filterChain.push(`[${lastOutput}][${currentInput}_scaled]overlay=${x}-w/2:${y}-h/2[${nextOutput}]`);
      } else {
        filterChain.push(`[${lastOutput}][${currentInput}]overlay=${x}-w/2:${y}-h/2[${nextOutput}]`);
      }
  ...
  let textFilterInput = imageStickers.length > 0 ? `v_img_${imageStickers.length - 1}` : `0:v`;
  
  textOverlays.forEach((overlay, index) => {
    const x = Math.round((overlay.x || 50) * 100); 
    const y = Math.round((overlay.y || 50) * 100); 
    const fontSize = overlay.fontSize || 50;
    const fontColor = overlay.fontColor || "white";
    const text = String(overlay.content)
      .replace(/'/g, "\\'") // Escape single quotes
      .replace(/"/g, '\\"');
      
    const drawTextFilter = `drawtext=text='${text}':x='(w*${x}/100)':y='(h*${y}/100)':fontsize=${fontSize}:fontcolor=${fontColor}:alpha=${overlay.opacity || 1}`;
    
    const outputLabel = `text_${index}`;
    if (imageStickers.length > 0 || index > 0) {
      filterChain.push(`[${textFilterInput}]${drawTextFilter}[${outputLabel}]`);
    } else {
      filterChain.push(`${drawTextFilter}`);
    }
    textFilterInput = outputLabel;
  });

  
  let filterComplex = filterChain.join(";");
  ...
  command += `-c:v libx264 -preset fast -c:a copy -y "${outputPath}"`;

  console.log(`🎬 [applyOverlaysToVideo] FFmpeg command: ${command.substring(0, 200)}...`);

  const session = await FFmpegKit.execute(command);
  const returnCode = await session.getReturnCode();

  if (ReturnCode.isSuccess(returnCode)) {
    console.log(`✅ [applyOverlaysToVideo] Overlays applied successfully`);
    return outputPath;
  } else {
    const failureStackTrace = (await session.getFailStackTrace()) || "Unknown overlay error";
    console.error(`❌ [applyOverlaysToVideo] FFmpeg failed: ${failureStackTrace}`);
