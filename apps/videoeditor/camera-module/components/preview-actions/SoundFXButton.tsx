...
import { listSoundFX } from "../../../src/api/soundFXService";
import type { SoundEffect } from "../../../src/types/soundEffect";
...
const SoundFXButton: React.FC<SoundFXButtonProps> = ({ onPress, onSoundSelect }) => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<"all" | "impact" | "transition" | "ambient">("all");

  
  useEffect(() => {
    if (showModal && soundEffects.length === 0) {
      loadSoundEffects();
    }
  }, [showModal]);
  ...
  const loadSoundEffects = async () => {
    setLoading(true);
    const effects = await listSoundFX();
  ...
  const filteredSounds =
    selectedCategory === "all"
      ? soundEffects
      : soundEffects.filter((s) => s.category === selectedCategory);
        ...
        <SafeAreaView style={styles.modalContainer}>
          ...
          {loading ? (
            <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
              ...
              <Text style={{ color: "#fff", marginTop: 12 }}>Loading sound effects...</Text>
            </View>
